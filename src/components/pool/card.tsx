...
export const PoolCard = (props: { pool: PoolInfo; account?: TokenAccount }) => {
  ...
  const handleSwitchViewModeRemove = () => {
    if (displayMode === "card") {
    ...
    >
      {displayMode === "card" && (
        <>
          ...
          <div className="pool-card-row">
            {props.account && (
              <Button type="default" onClick={handleSwitchViewModeAdd}>Add</Button>
            )}
            {props.account && (
              <Button onClick={handleSwitchViewModeRemove}>Remove</Button>
            )}
          </div>
        </>
      )}
